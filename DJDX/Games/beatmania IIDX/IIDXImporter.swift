...
actor IIDXImporter {
    ...
    static func insertSongRecord(database: Connection, record: IIDXSongRecord, importGroupID: String) {
        ...
        _ = try? database.run(col.songRecordTable.insert(
            [col.srImportGroupID <- importGroupID] + recordSetters(for: record)
        ))
    }
    ...
    static func updateSongRecord(database: Connection, id: Int64, record: IIDXSongRecord) {
        ...
        _ = try? database.run(row.update(recordSetters(for: record)))
    }

    // Column setters shared by insert and update (everything except the import
    // group id, which only applies on insert).
    // swiftlint:disable:next function_body_length
    static func recordSetters(for record: IIDXSongRecord) -> [Setter] {
        let col = IIDXPlayDataDatabase.self
        return [
