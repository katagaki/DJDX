...
struct PolarisChordWebImporter: View {
...
struct PolarisChordWebViewForImporter: UIViewRepresentable,
    ...
    func makeUIView(context: Context) -> WKWebView {
        ...
        webView.load(URLRequest(url: polarisChordVersion.loginPageRedirectURL()))
        #if DEBUG
...
class PolarisChordCoordinatorForImporter: NSObject, WKNavigationDelegate {
    let cleanupJS = """
\(globalJSFunctions)
...
"""
    ...
    func webView(_ webView: WKWebView, didFinish _: WKNavigation!) {
        ...
        webView.evaluateJavaScript(self.cleanupJS) { _, _ in
            if urlString.starts(with: self.version.musicDataPageURL().absoluteString) {
                webView.layer.opacity = 0.0
    ...
    func extractScoreData(from webView: WKWebView) {
        ...
        webView.callAsyncJavaScript(
            polarisChordScoreDataFetchBody,
            arguments: [:],
            in: nil,
            in: .page
        ) { result in
    ...
    func handleExtractionResult(_ value: String?) {
        #if DEBUG
        debugPrint("[PolarisChordImport] extraction result length:", value?.count ?? -1,
...
protocol PolarisChordUpdateScoreDataDelegate: Sendable {
